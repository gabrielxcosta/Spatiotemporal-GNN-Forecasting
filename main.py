...
# ----------------------------
# Inicializa CUDA e mostra informações
# ----------------------------
...
colors = ['#000000', '#EE2617', '#0E54B6', '#F2A241']  # Preto, vermelho, azul, amarelo
...
for dropout, hidden_size, num_layers in product(dropouts, embeddings, layers):
    ...
    with tqdm(range(num_epochs), desc=f"{config_name}", leave=True) as pbar:
        for epoch in pbar:
            model.train()
            epoch_loss = 0.0
            ...
            if early_stopper.step(val_loss, model):
                ...
                break
    print_time("Treinamento", t_train_start)
    ...
    print_time("Avaliação final", t_eval_start)
    print(f"{config_name}: RMSE {rmse:.4f} | MAE {mae:.4f} | R² {r2_corrected:.4f}")

    # ----------------------------
    # Plot Loss Train/Val
    # ----------------------------
    t_plot_1_start = time.time()
    ...
    plt.title(f'Loss Curve - {config_name}', fontsize=16)
    plt.legend(fontsize=12)
    plt.grid(linestyle=':')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f"loss_curve_{config_name}.png"), dpi=300, bbox_inches="tight")
    print_time("Plot 1", t_plot_1_start)

    # ----------------------------
    # Regplot Predito x Ground Truth
    # ----------------------------
    t_plot_2_start = time.time()
    plt.figure(figsize=(12,6), dpi=300)
    sns.regplot(
        x=y_true_test_mean,
        y=y_pred_test_mean,
        line_kws={"color":colors[1], "linewidth":2},
        scatter_kws={"alpha":0.5, "color":colors[1], "s":40}
    )
    plt.xlabel("True mean cases", fontsize=14)
    plt.ylabel("Predicted mean cases", fontsize=14)
    plt.title(f"Predicted vs True - {config_name}", fontsize=16)
    plt.grid(linestyle=':')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f"regplot_{config_name}.png"), dpi=300, bbox_inches="tight")
    print_time("Plot 2", t_plot_2_start)

    # ----------------------------
    # Plot final ajustado
    # ----------------------------
    t_plot_3_start = time.time()
    ...
    plt.plot(df['mean'], color=colors[0], label='Mean number of cases', linewidth=2)
    plt.fill_between(df.index, df['mean'] - df['std'], df['mean'] + df['std'],
                     color=colors[2], alpha=0.1, label='Std. deviation')
    plt.plot(range(train_size + val_size, total),
             y_pred_daily[train_size + val_size:], color=colors[1],
             label='Prediction', linewidth=2)
    plt.axvline(x=train_size, color='green', linestyle='--', linewidth=2)
    plt.text(train_size + 0.3, 2., 'Train/Val split', rotation=90,
             color='green', fontsize=10, verticalalignment='top')
    plt.axvline(x=train_size + val_size, color='blue', linestyle='--', linewidth=2)
    plt.text(train_size + val_size + 0.3, 2., 'Val/Test split', rotation=90,
             color='blue', fontsize=10, verticalalignment='top')
    plt.legend(fontsize=9, frameon=True, loc='lower center')
    plt.xlabel('Reports', fontsize=13)
    plt.ylabel('Mean number of cases', fontsize=13)
    plt.title(f'Prediction vs True Cases - {config_name}', fontsize=15)
    plt.grid(linestyle=':')
    plt.xticks(fontsize=11)
    plt.yticks(fontsize=11)
    plt.subplots_adjust(left=0.1, right=0.96, bottom=0.12, top=0.9)
    plt.savefig(os.path.join(results_dir, f"summary_plot_{config_name}.png"),
                dpi=200, bbox_inches="tight")
    plt.close()
    print_time("Plot 3", t_plot_3_start)

    # ----------------------------
    # Plot temporal (Pred vs True)
    # ----------------------------
    t_plot_4_start = time.time()
    plt.figure(figsize=(12,6), dpi=200)
    plt.plot(y_true_test_mean, label='True', color='black', linewidth=2)
    plt.plot(y_pred_test_mean, label='Predicted', color='red', alpha=0.8, linewidth=2)
    plt.xlabel('Test snapshots', fontsize=13)
    plt.ylabel('Mean number of cases', fontsize=13)
    plt.title(f'Temporal Comparison - {config_name}', fontsize=15)
    plt.legend(fontsize=10, frameon=True)
    plt.grid(linestyle=':')
    plt.xticks(fontsize=11)
    plt.yticks(fontsize=11)
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f"temporal_comparison_{config_name}.png"),
                dpi=200, bbox_inches="tight")
    plt.close()
    print_time("Plot 4 - Temporal Comparison", t_plot_4_start)

    print_time(f"Total {config_name}", t_model_start)

# ----------------------------
# Ranking final
# ----------------------------
print("\n=== Final Ranking (best configs) ===")
