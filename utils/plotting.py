...
def plot_summary(dataset, scaler, total, train_size, val_size,
    ...
    # Média móvel
    # --- FIX: moving average window must be int >= 1 ---
    ...
    plt.plot(df['moving_avg'], color='orange', linestyle='--',
             label=f'Moving Average ({ma_window})', linewidth=2.2)
    ...
    plt.text(train_size + 0.35, ymax * 1.05, 'Train/Val split', rotation=90,
             color='green', fontsize=10, verticalalignment='top')

    plt.axvline(x=train_size + val_size, color='blue', linestyle='--', linewidth=1.5)
    plt.text(train_size + val_size + 0.35, ymax * 1.05, 'Val/Test split', rotation=90,
...
def plot_loss(train_losses,val_losses,mode,results_dir):

    os.makedirs(results_dir,exist_ok=True)

    plt.figure(figsize=(10,6),dpi=200)

    plt.plot(train_losses,color="green",linewidth=2,label="Train")
    plt.plot(val_losses,color="blue",linewidth=2,label="Validation")
...
def plot_regression(y_true,y_pred,mode,results_dir):
    ...
    plt.savefig(os.path.join(results_dir,"regression.png"),dpi=200)
    plt.close()


def plot_temporal(y_true,y_pred,mode,results_dir):

    os.makedirs(results_dir,exist_ok=True)

    plt.figure(figsize=(12,6),dpi=200)

    plt.plot(y_true,color=COLORS[0],linewidth=2,label="True")
    plt.plot(y_pred,color=COLORS[1],linewidth=2,label="Predicted")

    plt.xlabel("Test Time")
    plt.ylabel("Mean Cases")

    plt.title(f"Temporal Comparison ({mode})")
    ...
    plt.savefig(os.path.join(results_dir,"temporal.png"),dpi=200)
    plt.close()